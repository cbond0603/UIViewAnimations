...
//  UIViewAnimations
//
//  Created by Chris Bond on 2/10/22.
//
...
//        UIView.animate(withDuration: 2.0, delay: 0.0, options: [.curveEaseInOut, .repeat, .autoreverse], animations: {            self.colorChangeButton.center.x = self.view.center.x}, completion: nil)
    ...
    @IBAction func pulsePressed(_ sender: UIButton) {
        let originalFrame = sender.frame // original Frame
        let widthShrink: CGFloat = 30
        let heightShrink: CGFloat = 10
        let smallerFrame = CGRect(x: sender.frame.origin.x + widthShrink,
                                  y: sender.frame.origin.y + heightShrink,width: sender.frame.width - (2 * widthShrink), height: sender.frame.height - (2 * heightShrink))
        ...
        let originalImageFrame = imageView.frame // original Frame
